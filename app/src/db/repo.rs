...
pub async fn get_trade_info(pool: &PgPool, security: &str) -> Vec<TradeInfo> {
    let sql = r#"
    select 
    ...
    from candles as c
    inner join securities as s on s.id = c.security_id
    inner join trades as t on t.security_id = c.security_id and c.begin_t = date_bin('1 min', t.trade_datetime, t.trade_datetime::date)
    where s.code = $1 
